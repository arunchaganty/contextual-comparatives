...
public class RankComparionsTest {
  ...
  private final GibbsSearcher gibbsSearcher = new GibbsSearcher(data, embeddings);
  ...
  public void testNasaBudget() {
    ...
    for (int i =0; i < 10; i++)
      log(gibbsSearcher.generate(mention).get(0).evaluate().comparisonNP(mention.value, mention.unit));
  }
